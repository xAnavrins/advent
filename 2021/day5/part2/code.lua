...
for _, point in ipairs(vents) do
	if point[1].x == point[2].x or point[1].y == point[2].y then
		...
		local dX = point[1].x < point[2].x and 1 or -1
		local dY = point[1].y < point[2].y and 1 or -1

		addPoint(map, point[1])

		while point[1].x ~= point[2].x and point[1].y ~= point[2].y do
			point[1].x = point[1].x + dX
			point[1].y = point[1].y + dY
			addPoint(map, point[1])
		end
	end
end
